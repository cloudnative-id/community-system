...
func (o *PatchSponsorURL) Build() (*url.URL, error) {
	...
	var _path = "/meetups/sponsors/{sponsor_id}"

	sponsorID := o.SponsorID
	if sponsorID != "" {
		_path = strings.Replace(_path, "{sponsor_id}", sponsorID, -1)
	} else {
		return nil, errors.New("sponsorId is required on PatchSponsorURL")
	}
...
func (o *PatchSponsorURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on PatchSponsorURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on PatchSponsorURL")
	}
